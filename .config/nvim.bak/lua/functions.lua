...
M.first_ecovim_run = function()
  local is_first_run = utils.file_exists('/tmp/first-myvim-run')

  if is_first_run then
    async.run(function()
      require('notify')("Welcome to MyVim! Hope you'll have a nice experience!", "info", { title = "Myvim", timeout = 5000 })
    ...
    local suc = os.remove('/tmp/first-myvim-run')
    if (not suc) then print("Error: Couldn't remove /tmp/first-myvim-run!") end
  end
end
